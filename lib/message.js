...
export async function getChatList(userId) {
  ...
  return chats.map(chatMember => {
    const otherMembers = chatMember.chat.members.filter(m => m.userId !== userId)
    const lastMessage = chatMember.chat.messages[0]
    ...
    return {
      ...
      unreadCount: 0 // You'll need to implement unread count logic
    }
  })
}
